...
public class RetroauthRobolectricRunner extends RobolectricTestRunner {
    /**
     ...
     */
    public RetroauthRobolectricRunner(Class<?> testClass) throws InitializationError {
        super(testClass);
    }
    ...
    public AndroidManifest create(final Config config) {
        ...
        if (FileFsFile.from(manifestPath).exists()) {
            manifest = FileFsFile.from(manifestPath);
    ...
    private String getManifestPath(Config config) {
        return null;
    }
}
